                ...
                <form action="{{ isset($user) ? route('users.update', $user->id) : route('users.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 xs:grid-cols-2 gap-4">
                        <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') || isset($user) ? $user->name : null }}" class="w-full text-sm text-gray-600 border border-gray-300 rounded-md h-8 focus">
                        </div>
                        @if (auth()->id() == 1)
                            <div>
                                <label for="mobile" class="block variable-font-medium text-sm text-gray-700 mb-1">@lang('موبایل')</label>
                                <input type="text" name="mobile" id="mobile" value="{{ old('mobile') || isset($user) ? $user->mobile : null }}" class="w-full text-sm text-gray-600 border border-gray-300 rounded-md h-8 focus">
                            </div>
                        @endif
                        <div>
                    ...
                    <div class="mt-4 flex justify-end">
                        <button type="submit" class="flex items-center justify-center text-sm bg-green-700 hover:bg-green-800 transition text-white focus-current ring-green-700 h-8 px-6 rounded-md">@lang(isset($user) ? 'ویرایش' : 'ساخت کاربر')</button>
                    </div>
                </form>
            </div>
        </div>
